...
def ilo_batch(model, scheduler, logdir, n = 100, begin = 0, end = -1, eta=0, dataset='celeba', task_config=None, device='cuda'):
    ...
    with open(log, "a") as f:
        print(f'DMILO-PGD: {len(image_paths)} Pictures', file = f)
    ...
    if task_name == 'inpainting':
        ...
        lr_z, lr_nu, lr_x = 2e-2, 1e-3, 2
        T = range(5)
        T_IN = range(500)
    elif task_name == 'nonlinear_deblur':
        lr_z, lr_nu, lr_x = 2e-2, 1e-3, 3e-1
        T = range(10)
        T_IN = range(200)
    ...
    for img_path in tqdm(image_paths, desc="Processing Images", unit="image"):
        ...
        plt.imsave(os.path.join(tmp_imgdir, 'init.png'), clear_color(X.clone()))
        # ILO Preparation
        ...
        for t_out in tqdm(T):
            ...
            if task_name == 'inpainting' or task_name == 'boxing':
                ...
                norm = torch.linalg.norm(operator.forward(output, mask=mask) - y_n)
                norm = torch.pow(norm, 2)
                norm_grad = torch.autograd.grad(outputs = norm, inputs = X)[0]
                X = X - lr_x * norm_grad
            elif task_name == 'super_resolution':
                output = torch.clamp(X, -1, 1)
                norm = torch.linalg.norm(operator.forward(output) - y_n)
                norm = torch.pow(norm, 2)
                norm_grad = torch.autograd.grad(outputs = norm, inputs = X)[0]
                X = X - lr_x * norm_grad
            elif task_name in ['gaussian_deblur', 'motion_deblur']:
                for _ in range(1000):
                    norm = torch.sqrt(torch.sum((operator.forward(X, kernel) - y_n) ** 2))
                    norm_grad_x = torch.autograd.grad(outputs = norm, inputs = X)[0]
                    X = X - lr_x * norm_grad_x
                    X = torch.clamp(X, -1, 1).detach().clone().requires_grad_(True)
                    X.grad = None
            else:
                for _ in range(1000):
                    norm = torch.sqrt(torch.sum((operator.forward(X) - y_n) ** 2))
                    norm_grad = torch.autograd.grad(outputs = norm, inputs = X)[0]
                    X = X - lr_x * norm_grad
                    X = torch.clamp(X, -1, 1).detach().clone().requires_grad_(True)
                    X.grad = None
            ...
            plt.imsave(os.path.join(tmp_imgdir, f'{t_out}_before.png'), clear_color(tar_X.clone()))
            if task_name == 'inpainting' or task_name =='boxing':
                measure_tar_X = operator.forward(tar_X, mask = mask)
            elif task_name in ['gaussian_deblur', 'motion_deblur']:
                measure_tar_X = operator.forward(tar_X, kernel=kernel)
            else:
                measure_tar_X = operator.forward(tar_X)
            ##### End Preparation
            ##### Begin ILO
            # ILO preparation
            target_latents[sample_step] = measure_tar_X
            # End Preparation
            ...
            with torch.no_grad():
                tar_X = X.detach().clone().requires_grad_(False)
                plt.imsave(os.path.join(tmp_imgdir, f'{t_out}_after.png'), clear_color(tar_X.clone()))
            ##### End Test Current X
            ##### End Out Loop
            #### Record Information
            with torch.no_grad():
